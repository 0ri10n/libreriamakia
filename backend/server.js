...
app.post('/register', async (req, res) => {
    ...
    } catch (err) { res.status(500).json({ msg: 'Error de base de datos' }); }
});
...
app.post('/login', async (req, res) => {
    ...
    try {
        ...
        const token = jwt.sign(payload, process.env.JWT_SECRET, { expiresIn: '2h' });
        res.json({ token });
    } catch (err) { res.status(500).json({ msg: 'Error en el servidor' }); }
});
...
app.post('/api/loans', proteger, async (req, res) => {
    try {
        const loan = new Loan({ 
            ...
            returnDate: new Date(Date.now() + 15 * 24 * 60 * 60 * 1000) // 15 días
        });
        await loan.save();
